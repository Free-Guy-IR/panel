...
def _fail_first_persist(monkeypatch: pytest.MonkeyPatch, delay: float = 0.0) -> dict[str, int]:
...
async def test_a_node_already_reset_is_not_cancelled_when_persistence_stops(
    ...
    assert billed[user_ids[1]] == 700
    assert billed[user_ids[2]] == 50
    assert charted[user_ids[1]] == billed[user_ids[1]]
    assert charted[user_ids[2]] == billed[user_ids[2]]
...
async def test_a_node_read_during_a_failing_persist_is_kept_for_the_next_cycle(
    ...
    billed, charted = await usage_snapshot(recorder_db, user_ids)
    assert billed[user_ids[1]] == 900
    assert charted[user_ids[1]] == 900
...
async def test_kept_bytes_survive_a_cycle_whose_database_read_fails(monkeypatch: pytest.MonkeyPatch, recorder_db):
    ...
    billed, charted = await usage_snapshot(recorder_db, user_ids)
    assert billed[user_ids[1]] == 250
    assert charted[user_ids[1]] == 250
