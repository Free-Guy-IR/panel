...
5. **History** (`experiments/02_history_purge.py`): after the flush, `GET /history?start=now-1h&end=now&username=demo-kids` lists the destinations with `hits` and first/last seen; `start=now-3d` → 422 with the two-day message (FR-006/007/008).
...
8. **Scoping** (`experiments/03_scoping.py`): create a non-sudo admin with a role granting `nodes.logs`, owning `demo-open` only; its `/history` and `/live` never contain `demo-kids` rows; `PUT /settings` → 403 (SC-007, FR-014).
