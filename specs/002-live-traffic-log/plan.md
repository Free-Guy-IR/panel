...
**Storage**: the panel database (SQLite on the test server, MySQL in production, PostgreSQL supported upstream) — three new fork tables via one Alembic revision on head `f6c4d0e2a183`
...
**Constraints**: node log capture is best-effort (10,000-line node buffer, non-blocking); one shared channel per node; no node config change; no core restart; 48 h retention; record ceiling default 2,000,000; no source IP stored; no explanatory comments; no new TS errors
...
| VI. Security and Privacy | `nodes/logs` permission on every endpoint; non-sudo scoped by `admin_id`; no source IP stored; 48 h purge plus ceiling; tokens only over loopback/ssh in tests. |
...
5. **Retention + ceiling.** Purge job every 10 min: delete `bucket_start < now − 48 h` in 5,000-id chunks up to 200,000 per run; then delete ids below `max(id) − ceiling` the same way. `max−ceiling` over-estimates the count only in the safe direction.
6. **Access.** `require_permission("nodes","logs")` on every route; `admin.is_sudo` sees all, otherwise `identities.admin_id == admin.id`; unowned rows are sudo-only; settings toggle sudo-only.
